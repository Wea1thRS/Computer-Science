...
public class ExtraCredit1 
{
    ...
    */
        ...
        switch (county) {
            case "Dale":
                taxRate = 1.5;
                if(product.toLowerCase().equals("alcohol") || product.toLowerCase().equals("tobacco")) {
                    if(paymentMethod.toLowerCase().equals("cash")) 
                        taxRate += 5.0;
                    taxRate += 2.0;
                }
                else if(product.toLowerCase().equals("potato chips") || product.toLowerCase().equals("soda pop")) {
                    taxRate += 1.5;
                } 
                if(paymentMethod.toLowerCase().equals("check"))
                    taxRate += 1.0;
                return taxRate;
            case "Richland":
                taxRate = 3.0;
                if(product.toLowerCase().equals("books"))
                    return 1.0;
                if(product.toLowerCase().equals("shoes"))
                    taxRate += 2.0;
                else if(product.toLowerCase().equals("clothing"))
                    taxRate += 1.0;
                if(paymentMethod.toLowerCase().equals("credit"))
                    taxRate += 1.0;
                else if(paymentMethod.toLowerCase().equals("check"))
                    taxRate += 0.5;
                return taxRate;
            case "Park":
                taxRate = 2.0;
                if(!product.toLowerCase().equals("food")) {
                    if(product.toLowerCase().equals("alcohol"))
                        return 5.0;
                    if(product.toLowerCase().equals("shoes") || product.toLowerCase().equals("clothing"))
                        return 0.0;
                    taxRate += 1.0;
                }
                if(paymentMethod.toLowerCase().equals("cash") || paymentMethod.toLowerCase().equals("credit"))
                    taxRate -= 1.0;
                return taxRate;
            default:
                break;
